...
분야 좌표는 Ch.1에서 그렸다. 리뷰 한 편과 인용 그래프, hypothesis placeholder 한 줄까지가 그 결과물이다. 좌표가 잡힌 학생이 다음으로 마주하는 막힘은 모양이 다르다. 지도교수가 "SLAM 정확도를 높여 보자"라고 말했는데, 그 한 문장에서 어디로 가야 손이 움직이는지 보이지 않는 단계다. 분야 지도가 있다고 풀어야 할 문제까지 자동으로 정해지지는 않는다.
...
좁히기가 어떻게 이루어지는지를 한 사례로 본다. 출발은 지도교수가 던진 흔한 한 줄, "SLAM 정확도를 높이고 싶다"이다. 이 문장은 objective는 있지만 problem statement는 아니다. 무엇을, 어떤 조건에서, 어떻게 측정해 좋아졌다고 말할지가 비어 있다.
...
Ch.1에서 stub로 잡아 둔 hypothesis placeholder가 이 장의 좁히기 절차를 거쳐 한 단락짜리 problem statement로 자랐다. 그 단락이 본인 종이 위에 적힌 시점에서 학생의 화법이 바뀐다. "잘 안 되네요"가 "이 조건에서 이렇게 해볼까요"로 바뀐다. 그 전환이 일어나는 시점이 곧 주제의 주도권이 지도교수에서 학생으로 넘어오는 시점이다. 다음 챕터에서 그 전환을 본다.
