...
heading은 자동 생성되는 TOC고, 그 TOC가 곧 논문의 골격이다. Whitesides가 §III에 적은 권고를 다른 표현으로 옮기면 *heading만 읽어도 논문의 main message가 재구성되어야 한다*가 된다. 이 조건이 깨진 논문은 skim 단계에서 reject 사유를 만든다.
...
- Conclusions에 future work 한 줄만 달랑 쓰는 경우 — higher-level analysis가 비어 있다. future work는 Conclusions의 *마지막 phrase*에만 들어간다.
