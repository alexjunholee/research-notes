...
한 단락도 잘 닫지 못하는 학생이 논문 전체를 닫아야 한다는 요구를 받는다. 닫는 문제가 같은 형태로 세 번 반복된다는 점만 보이면 처방은 한 가지로 줄어든다. [Mensh & Kording 2017](https://doi.org/10.1371/journal.pcbi.1005619) rule 3이 그 한 가지를 Context-Content-Conclusion(CCC)으로 정식화했다.
...
가이드의 입장은 둘 다 끝을 노린다는 한 줄로 정리된다. 스케일이 다르고 끝이 가리키는 단위도 다르지만, 독자의 기대 방향은 같다 — 충돌하지 않는 두 규칙이 같은 문장 끝에서 만난다.
...
**출처.** [Mensh & Kording 2017](https://doi.org/10.1371/journal.pcbi.1005619) rule 3·4·5. [Gopen & Swan 1990](https://www.americanscientist.org/sites/americanscientist.org/files/20053191534_306.pdf) (stress position). tension 화해 입장은 자체.
